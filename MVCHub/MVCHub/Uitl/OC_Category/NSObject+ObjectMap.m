        ...
        if ([nestedArray[xx] isKindOfClass:[NSDictionary class]]) {
            ...
            for (NSString *newKey in [nestedArray[xx] allKeys]) {
                ...
                // If it's a Dictionary, create an object, and send to [self objectFromJSON]
                    ...
                    id nestedDictObj;
                    if ([type isEqualToString:@"NSDictionary"] || [type isEqualToString:@"NSArray"]) { // 如果对象名是一个基本对象而非自创对象
                        nestedDictObj = [nestedArray[xx] objectForKey:newKey];
                    }else{
                        nestedDictObj = [NSObject objectOfClass:type fromJSON:[nestedArray[xx] objectForKey:newKey]];
                    }
